...
export default function SalesCatalogModule({ 
  ...
  // Intelligent categorization stats with item counts
  const categoryStats = useMemo(() => {
    // 1. Group by exact item.category strings
    const countsByExactName = new Map<string, number>();
    items.forEach(item => {
      const cat = (item.category || 'General').trim();
      countsByExactName.set(cat, (countsByExactName.get(cat) || 0) + 1);
    });

    // 2. Categories directly found in current items
    const inCatalogCats = Array.from(countsByExactName.entries())
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));

    // 3. For registered categories, calculate matching count using doesItemMatchCategory
    const registeredWithMatches = categories.map(c => {
      const count = items.filter(item =>
        doesItemMatchCategory(item.category, c.name, item.itemCode, item.subcategory)
      ).length;
      return {
        id: c.id,
        code: c.code,
    ...
    // Build unified active list of categories that contain products
    const activeList: Array<{ name: string; count: number }> = [];
    const seenNames = new Set<string>();

    inCatalogCats.forEach(c => {
      const key = c.name.toLowerCase().trim();
      seenNames.add(key);
      activeList.push(c);
    });

    registeredWithMatches
      .filter(c => c.count > 0)
      .forEach(c => {
        const key = c.name.toLowerCase().trim();
        if (!seenNames.has(key)) {
          seenNames.add(key);
          activeList.push({ name: c.name, count: c.count });
        }
      });

    // Registered categories that currently have 0 matching products
    const emptyList = registeredWithMatches.filter(
      c => c.count === 0 && !seenNames.has(c.name.toLowerCase().trim())
    );
    ...
    return {
      ...
      emptyList,
      inCatalogCats,
      countsMap: Object.fromEntries(countsByExactName)
    };
  }, [items, categories]);
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      if (categoryFilter !== 'all' && !doesItemMatchCategory(item.category, categoryFilter, item.itemCode, item.subcategory)) return false;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
          ...
          <div className="md:col-span-3">
            ...
            >
              <option value="all">
                📂 Todas las Clases ({categoryStats.activeList.length} activas · {items.length} productos)
              </option>

              {/* Classes with products in this catalog */}
              {categoryStats.activeList.length > 0 && (
                <optgroup label={`⭐ Clases con Productos en este Catálogo (${categoryStats.activeList.length})`}>
                  {categoryStats.activeList.map(cat => (
                    <option key={cat.name} value={cat.name}>
                      🟢 {cat.name} ({cat.count} {cat.count === 1 ? 'producto' : 'productos'})
                    </option>
                  ))}
                </optgroup>
              )}

              {/* Other standard registered classes */}
              {categoryStats.emptyList.length > 0 && (
                <optgroup label={`📋 Otras Clases Disponibles (${categoryStats.emptyList.length})`}>
                  {categoryStats.emptyList.map(cat => (
        ...
        {(typeFilter !== 'all' || statusFilter !== 'all' || stockFilter !== 'all' || categoryFilter !== 'all' || clientFilter !== 'all' || searchQuery) && (
          <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100 text-xs">
            ...
            {categoryFilter !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-sky-50 text-[#0196C1] font-bold border border-sky-200">
                Clase: {categoryFilter} ({filteredItems.length} {filteredItems.length === 1 ? 'producto' : 'productos'})
                <button onClick={() => setCategoryFilter('all')} className="cursor-pointer hover:text-black font-bold ml-1">×</button>
              </span>
            )}
            {typeFilter !== 'all' && (
        ...
        {filteredItems.length === 0 ? (
          <div className="p-10 text-center space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-800">
              {categoryFilter !== 'all' 
                ? `No hay artículos asignados a "${categoryFilter}"`
                : 'No se encontraron artículos en el catálogo'}
            </h3>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              {categoryFilter !== 'all' ? (
                <span>
                  No hay productos registrados con esta clase en el catálogo actual ({items.length} productos disponibles en otras clases).
                </span>
              ) : (
                <span>
            ...
            {/* Suggestions of active categories if a filter yielded 0 */}
            {categoryFilter !== 'all' && categoryStats.activeList.length > 0 && (
              <div className="pt-2">
                <span className="text-[11px] text-slate-400 font-semibold block mb-2">
                  Clases con productos disponibles en este catálogo:
                </span>
                <div className="flex flex-wrap justify-center gap-1.5 max-w-lg mx-auto">
                  {categoryStats.activeList.slice(0, 6).map(c => (
                    <button
                      key={c.name}
                      onClick={() => setCategoryFilter(c.name)}
                      className="px-2.5 py-1 bg-sky-50 hover:bg-sky-100 text-[#0196C1] border border-sky-200 text-xs font-semibold rounded-lg cursor-pointer transition-colors"
                    >
                      {c.name} ({c.count})
                    </button>
                  ))}
            ...
            <div className="pt-2 flex flex-wrap justify-center gap-2">
              {categoryFilter !== 'all' && (
                <button
                  ...
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold rounded-xl cursor-pointer shadow-xs flex items-center gap-1.5"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>Ver Todas las Clases ({items.length})</span>
                </button>
              )}
              <button
                onClick={() => setShowImportModal(true)}
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setCategoryFilter(item.category || 'General');
                          }}
                          className="group/cat flex items-center gap-1 text-left cursor-pointer"
                          title={`Filtrar solo por: ${item.category || 'General'}`}
                        >
                          <span className="font-bold text-slate-800 text-[11px] block line-clamp-1 group-hover/cat:text-[#0196C1] group-hover/cat:underline transition-colors">
                            {item.category || 'General'}
                          </span>
                          <Filter className="w-2.5 h-2.5 opacity-0 group-hover/cat:opacity-100 text-[#0196C1] transition-opacity shrink-0" />
                        </button>
                        {item.subcategory && (
      ...
      <CatalogCategoriesModal
        ...
        itemCountsByCategory={categoryStats.countsMap}
      />
